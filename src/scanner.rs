...
fn getkeywords_hashmap() -> HashMap<&'static str, TokenType> {
    HashMap::from([
        ("e", And),
        ("classe", Class),
        ("senao", Else),
...
impl Scanner {
        ...
        match c {
            ...
            '|' => {
                if self.char_match('>') {
                    ...
                    return Err(format!("Faltou o símbolo '>' na linha {}", self.line));
                }
            },
            ' ' | '\r' | '\t' => {}
            '\n' => self.line += 1,
            ...
            c => {
                if is_digit(c) {
                    ...
                    return Err(format!("Caractere não reconhecido na linha {}: {}", self.line, c));
                }
            }
        }
    ...
    fn number(self: &mut Self) -> Result<(), String> {
        ...
        match value {
            ...
            Err(_) => return Err(format!("Não foi possível converter o número: {}", substring)),
        }
    ...
    fn string(self: &mut Self) -> Result<(), String> {
        ...
        if self.is_at_end() {
            return Err("Texto sem marcador de término".to_string());
        }
...
mod tests {
    ...
    #[test]
    fn getkeywords() {
        let source = "var valor = 12;\nenquanto verdadeiro { saida 3 };";
        let mut scanner = Scanner::new(source);
        scanner.scan_tokens().unwrap();
